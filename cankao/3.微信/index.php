...
/*********************实现翻译的流程***********************************************************/
    ...
    { 
        ...
        if (!empty($postStr)){
            ...
            if(!empty( $keyword ))
            {
                ...
                $str_valid = mb_substr($keyword,2,202,"UTF-8");
                if($str_trans == 'fy' && !empty($str_valid)){
        
                    $word = mb_substr($keyword,2,202,"UTF-8");
                    //调用有道词典
                    //$contentStr = $this->youdaoDic($word);
                    //调用百度词典
                    $res =$this->translate($word,'auto','auto');
                    $contentStr=$res['trans_result']['0']['dst'];
                }else {
                    $contentStr = "感谢您关注翔哥的公众号 ，更多功能以后将推出";
                }
    ...
    public function translate($query, $from, $to)
    {
        ...
        $args = array(
            'q' => $query,
            'appid' => '20161214000034027',
            'salt' => rand(10000,99999),
            'from' => $from,
            'to' => $to,
    
        );
        $args['sign'] = $this->buildSign($query, '20161214000034027', $args['salt'], "ZoJ38zpKXudswQDS9dJa");
